...
def ipr_aceite_darcy_empirico(J, Pr, Pwf):
    return enforce_q_positive(J * (Pr - Pwf))

def ipr_aceite_darcy_ynf(Km, Kf, poro_m, poro_f, h, Pr, Pwf, Bo, mu, Re, Rw, S, system='english'):
    ...
    q = (c * k_eq * (Pr - Pwf)) / divisor
    return enforce_q_positive(np.where(divisor != 0, q, np.nan))

def ipr_aceite_vogel(Qtest, Pwf_test, Pr, Pwf):
    rel_test = np.where(Pr != 0, Pwf_test / Pr, 0)
    qmax = Qtest / (1.0 - 0.2 * rel_test - 0.8 * rel_test**2)
    rel_p = np.where(Pr != 0, Pwf / Pr, 0)
    q = qmax * (1.0 - 0.2 * rel_p - 0.8 * rel_p**2)
    return enforce_q_positive(q)

def ipr_aceite_babu_odeh(Kx, Kz, b, A, Pr, Pwf, Bo, mu, rw, system='english'):
    divisor = mu * Bo * (np.log(np.sqrt(A) / rw) - 0.75)
    c = 0.00708 if system == 'english' else 0.0525
    q = (c * b * np.sqrt(Kx * Kz) * (Pr - Pwf)) / divisor
    return enforce_q_positive(np.where(divisor != 0, q, np.nan))

def ipr_aceite_joshi(Kh, Kv, h, L, Pr, Pwf, Bo, mu, re, rw, system='english'):
    beta = np.sqrt(Kh / Kv)
    ...
    q = (c * Kh * h * (Pr - Pwf)) / divisor
    return enforce_q_positive(np.where(divisor != 0, q, np.nan))

def ipr_gas_pseudo_estable(Kg, h, Pr, Pwf, mu_g, Z, T, re, rw, S, system='english'):
    divisor = mu_g * Z * T * (np.log(re / rw) - 0.75 + S)
    c = 0.000703 if system == 'english' else 0.00424
    q = (c * Kg * h * (Pr**2 - Pwf**2)) / divisor
    return enforce_q_positive(np.where(divisor != 0, q, np.nan))

def ipr_gas_economides(Kg, Kf, wf, xf, h, Pr, Pwf, mu_g, Z, T, re, rw, system='english'):
    Sf = -np.log((xf / 2.0) / rw)
    return ipr_gas_pseudo_estable(Kg, h, Pr, Pwf, mu_g, Z, T, re, rw, Sf, system)

def ipr_gas_joshi_horizontal(Kh, Kv, h, L, Pr, Pwf, mu_g, Z, T, re, rw, S=0.0, system='english'):
    beta = np.sqrt(Kh / Kv)
    a = (L / 2.0) * (0.5 + np.sqrt(0.25 + (re / (L/2.0))**4))**0.5
    t1 = np.log((a + np.sqrt(a**2 - (L/2.0)**2)) / (L/2.0))
    t2 = (beta * h / L) * np.log(beta * h / (2 * rw))
    ...
    c = 0.000703 if system == 'english' else 0.00424
    q = (c * Kh * h * (Pr**2 - Pwf**2)) / divisor
    return enforce_q_positive(np.where(divisor != 0, q, np.nan))

def ipr_gas_ynf(Km, Kf, poro_m, poro_f, h, Pr, Pwf, mu_g, Z, T, re, rw, S, system='english'):
    coef_part = 1.0 - (poro_m / (poro_m + poro_f))
    k_eq = Km * h * (1.0 - coef_part) + Kf * h 
    divisor = mu_g * Z * T * (np.log(0.472 * (re / rw)) + S)
    c = 0.000703 if system == 'english' else 0.00424
    q = (c * k_eq * (Pr**2 - Pwf**2)) / divisor
    return enforce_q_positive(np.where(divisor != 0, q, np.nan))
